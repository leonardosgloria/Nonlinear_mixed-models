...
modelZ=nlme(w~a/(1+b*exp(-k*age)), fixed=list(a+b+k ~CG-1 ),random=a+k~1,
               start=stv
               ,data = data_GWAS)
```
...
```{r}
fixM_k=c(a~ factor(CG)-1,b~factor(CG)-1,k ~ factor(CG)-1+M) # fixed effects with SNP

fixM_a=c(a~ factor(CG)-1+M,b~factor(CG)-1,k ~ factor(CG)-1) # fixed effects with SNP

fixM_ak=c(a~ factor(CG)-1+M,b~factor(CG)-1,k ~ factor(CG)-1+M) # fixed effects with SNP


stv2=c(rep(57,nfl),0,rep(29,nfl),rep(0.04,nfl))

pM=GWASnlme(data=data_GWAS,M=M,fixedM=fixM_a,startM=stv2,
            model0=modelZ,fdrlevel=0.05,lambda=0.5,ncores=16,parallel = T)
